...
class MoveRangePreview {
    constructor(options, gamemap, terrainInfo, players) {
    ...
    updateIfNeeded() {
        ...
        if (this.selectedUnit !== undefined) {
            this.clearMoveRange();
            let showNextDayMove =
                (this.options.options_next_day_range_preview_mode === NextDayRangePreviewMode.ALWAYS_ON)
                || (this.options.options_next_day_range_preview_mode === NextDayRangePreviewMode.WHEN_HELD
                    && this.cursorData.isQuickMoveRange);
            let showNextDayAttack =
                (this.options.options_next_day_range_preview_mode === NextDayRangePreviewMode.ALWAYS_ON
                || this.options.options_next_day_range_preview_mode === NextDayRangePreviewMode.WHEN_HELD)
                && this.cursorData.isQuickAttackRange;

            if (showNextDayAttack) {
                this.showMoveAndAttackRangeFor(this.selectedUnit);
            } else {
                this.showMoveRangeFor(this.selectedUnit, showNextDayMove);
            }
            return;
        }
