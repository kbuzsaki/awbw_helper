    ...
    getSelectedDataset() {
        ...
        let aggregation = {
            ...
            "Day Average (End)": {
                title: "Day (Average)",
                labelFn: ({turn}) => {
                    let sortedPlayers = Object.values(turn.gameState.players).sort(
                        (lhs, rhs) => lhs.players_order - rhs.players_order);
                    let firstPlayer = sortedPlayers[0];
                    let currentPlayer = turn.gameState.players[turn.gameState.currentTurnPId];
                    if (currentPlayer.players_id === firstPlayer.players_id) {
                        return turn.day - 1;
                    }
                    return turn.day;
                },
