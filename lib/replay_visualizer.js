...
<div class="awbwenhancements-replay-visualizer">
    <div class="awbwenhancements-replay-visualizer-toolbar">
        ...
        <select class="awbwenhancements-select awbwenhancements-replay-visualizer-metric-select">
            ...
            <option>Total Funds Generated</option>
...
function makeCopImage() {
    var img = new Image();
    img.src = "https://awbw.amarriner.com/terrain/aw2/redstar.gif";
    return img;
}

function makeScopImage() {
    var img = new Image();
    img.src = "https://awbw.amarriner.com/terrain/aw2/bluestar.gif";
    return img;
}
...
class ReplayVisualizer {
    ...
    getSelectedDataset() {
        ...
        let numPlayers = Object.keys(this.replayState[0].gameState.players).length;
        ...
        let res = this.getDataset(replayMetrics, metric, aggregation);
        res.annotations = this.getPowerAnnotations(replayMetrics, aggregation);
        return res;
    }

    getDataset(replayMetrics, metric, aggregation) {
    ...
    getPowerAnnotations(replayMetrics, aggregation) {
        let boxes = [];
        for (let turnId in replayMetrics) {
            let turn = replayMetrics[turnId];
            let turnIndex = parseFloat(turnId);
            let xMin = turnIndex / aggregation.dayFactor;
            let xMax = turnIndex / aggregation.dayFactor + aggregation.dayIncrement;

            let colors = kCountryLabelColors[turn.active_country] || {};

            let playerMetrics = turn.player_metrics[turn.active_player_id];
            let turnMetrics = aggregation.selectFn({playerMetrics});
            if (turnMetrics.co_power === "Y") {
                boxes.push({
                    type: 'box', xMin, xMax,
                    backgroundColor: rgbWithOpacity(colors.fill, 0.3),
                    borderColor: colors.border,
                    label: {
                        display: true,
                        content: makeCopImage(),
                        width: 14,
                        height: 14,
                        position: {x: 'center', y: 'end'},
                    },
                });
            } else if (turnMetrics.co_power === "S") {
                boxes.push({
                    type: 'box', xMin, xMax,
                    backgroundColor: rgbWithOpacity(colors.fill, 0.3),
                    borderColor: colors.border,
    ...
    initializeVisualization() {
        ...
        let {title, labels, annotations, datasets} = this.getSelectedDataset();

        let chartConfig = {
            type: 'line',
            data: {labels, datasets, annotations},
            options: {
                scales: {
                    y: { beginAtZero: true }
                },
