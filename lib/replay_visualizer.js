...
class ReplayVisualizer {
    ...
    getSelectedDataset() {
        ...
        return this.getDataset(metricDefinition);
    }
    ...
    getDataset(metric) {
        ...
        for (let turn of this.replayState) {
            ...
            labels.push(turn.day + " - " + currentPlayer.countries_code);

            for (let player of Object.values(turn.gameState.players)) {
                let dataset = datasetMap[player.players_id];
                dataset.data.push(metric.fn({turn, player}));
            }
        }

        let title = metric.title + " by Turn";
