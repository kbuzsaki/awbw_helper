getReplayMetricsByTurn = (function() {
    function getTurnStateMetrics(turnState, playerId, actions) {
        ...
        return {
            ...
            co_power_on: player.players_co_power_on,
        };
    }

    function getPowerState(actions) {
        for (let action of actions) {
            if (action.action === "Power") {
                return action.coPower;
            }
        }
        return "N";
    }
    ...
    function analyzePlayerTurnStart(turnState, turns, index, playerId) {
        ...
        for (let i = 0; i < turns.length; i++) {
            ...
            // Intentionally use coercing equals because playerId is a string but currentTurnPId is a number
        ...
        metrics.co_power = getPowerState(turns[index].actions);
        return metrics;
    }
        ...
        for (let action of turn.actions) {
            ...
            if (action.action === "Move"
             ...
             || action.action === "Resign") {
                ...
                if (action.newIncome === undefined) {
                    continue;
                }

                var affectedPlayers = {};
                if ('player' in action.newIncome) {
                    affectedPlayers["" + action.newIncome.player] = action.newIncome;
                } else {
                    affectedPlayers = action.newIncome;
                }
                for (let key in affectedPlayers) {
                    let affectedPlayer = affectedPlayers[key];
                    let playerId = affectedPlayer.player;
                    let newIncome = affectedPlayer.income;
                    gameState.players[playerId].players_income = newIncome;
                }
