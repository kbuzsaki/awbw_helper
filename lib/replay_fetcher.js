...
class ReplayFetcher {
    ...
    fetchReplay(gameId, progressUpdateCb) {
        return Promise.all([
        ...
        .then(([firstTurnState, cachedTurns]) => {
            ...
            let progress = numCachedTurns;
            let total = numTurns;
            progressUpdateCb(progress, total);

            let fetchedTurnPromises = [];
            for (let turn = numCachedTurns; turn < numTurns; turn++) {
                fetchedTurnPromises.push(this._fetchReplayStateForTurn(gameId, turn).then((res) => {
                    progress += 1;
                    progressUpdateCb(progress, total);
                    return res;
                }));
