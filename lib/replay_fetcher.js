...
// TODO: Cache needs to be smarter about size limitations
const kReplayFetcherCacheLimit = 1;
    ...
    _getCachedReplayState(gameId) {
        return new Promise((resolve) => {
            ...
            chrome.storage.local.get(request, (result) => {
                ...
                resolve(cacheEntry.replayState);
            });
        });
    }
    ...
    _setCachedReplayState(gameId, allTurns) {
        ...
        chrome.storage.local.get(request, (result) => {
            ...
            cache[gameId] = {
                ...
                replayState: allTurns.slice(0, allTurns.length - 1),
                lastUsedMs: Date.now(),
            };
            ...
            console.log("Storing cache:", cache);
            chrome.storage.local.set(result);
        });
    }
